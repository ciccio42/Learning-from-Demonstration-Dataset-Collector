...
class TrajectoryCollector():
    ...
    def __init__(self, env_camera_name: list, env_camera_topic_name: str, gripper_state_topic: str, joint_state_topic: str, trj_state_topic: str, tcp_frame_name: str, frame_rate: int, saving_folder: str, task_name: str, task_id: str, home_pos: list):
        ...
        self._env_camera_service_client = rospy.ServiceProxy(env_camera_topic_name, GetFrames, True)
        ...
        while exception:
            try:
                # Get TCP Pose
                tcp_pose = self._tfBuffer.lookup_transform('base_link', self._tcp_frame_name, rospy.Time())
    ...
    def _save_step(self, env_frames: GetFramesResponse, tcp_pose: TransformStamped, joint_state: JointState, state: str, gripper_action: int, gripper_q_pos: int, done: int, reward: int):
        ...
        # fill observations        
        obs = dict()
        obs[TrajectoryCollector.EEF_POS]=np.array([tcp_pose.transform.translation.x, tcp_pose.transform.translation.y, tcp_pose.transform.translation.z])
        obs[TrajectoryCollector.EEF_QUAT]=np.array([tcp_pose.transform.rotation.x, tcp_pose.transform.rotation.y, tcp_pose.transform.rotation.z, tcp_pose.transform.rotation.w])
        ...
        obs[TrajectoryCollector.EE_AA]=self._quat2axisangle(obs[TrajectoryCollector.EEF_QUAT])
        ...
        for j, (color_msg, depth_msg) in enumerate(zip(env_frames.color_frames,env_frames.depth_frames)):
            color_cv_image = self._bridge.imgmsg_to_cv2(color_msg, desired_encoding='bgr8')
            depth_cv_image = self._bridge.imgmsg_to_cv2(depth_msg, desired_encoding='mono16')
        ...
        self._trajectory.append(obs=obs, reward=reward, done=done, info=info, action=action)
    ...
    def _save_trajectory(self):
        file_name = os.path.join(self._saving_folder, 'traj{:03d}.pkl'.format(self._trajectory_cnt))
    ...
    def get_message(self):
        ...
        # Get joint states
        joint_state = rospy.wait_for_message(topic=self._joint_state_topic, topic_type=JointState)
        
        # Get env_frames
        env_frames = self._env_camera_service_client()
        
        # Get gripper state
        gripper_state = rospy.wait_for_message(topic=self._gripper_state_topic, topic_type=GripperState)

        # Get the trajectory state
        trajectory_state = rospy.wait_for_message(topic=self._trj_state_topic, topic_type=TrajectoryState)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.init_trj()
            rospy.loginfo(f"Collecting task {self._task_name} - Sub task {self._task_id} - Counter {self._trajectory_cnt}")
